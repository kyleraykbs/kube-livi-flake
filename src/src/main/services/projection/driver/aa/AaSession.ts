...
export class AaSession extends EventEmitter implements IPhoneDriver {
  ...
  /**
   * A keystroke from a physical keyboard. `sendButton` already takes any
   * keycode, so typing needs no new wire format - only a down/up pair per
   * keystroke, which is what the phone's focused text field expects.
   */
