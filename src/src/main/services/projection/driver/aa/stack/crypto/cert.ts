// Standard Android Auto Head Unit certificate, signed by "Google Automotive Link" CA.
// This is the canonical open-source AA HU certificate used by OpenAuto.
// Android Auto phones recognise and accept this certificate during the TLS handshake.
// Source: aasdk (opencardev / f1x.studio) transport::ISSLWrapper embedded cert.
export const HU_CERT_PEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`

export const HU_KEY_PEM = `-----BEGIN RSA PRIVATE KEY-----

KAwp3tIHPoJOQiKNQ3/qks5km/9dujUGU2ARiU3qmxLMdgegFz8e
-----END RSA PRIVATE KEY-----`
