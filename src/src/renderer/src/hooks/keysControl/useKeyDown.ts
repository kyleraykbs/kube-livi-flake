...
import { rawKeyCommand } from '@shared/types/InputCommand'
import { androidKeycodeForDomCode } from '@shared/types/KeyboardInput'
...
export const useKeyDown = ({
  ...
  return useCallback(
    (event: KeyboardEvent) => {
      ...
      // Typing. An unbound printable key goes to the phone as the key it is, so
      // a physical keyboard types into the phone's text boxes; keys that are
      // bound to a command stay commands, and modifier chords are left to the
      // UI/browser. The phone only accepts keycodes the head unit advertised in
      // the SDR, which is where TYPING_KEYCODES comes in.
      if (
        !mappedAction &&
        receivingVideo &&
        !formFocused &&
        ...
        const keyCode = androidKeycodeForDomCode(code)
        if (keyCode !== null) {
          onSetKeyCommand(rawKeyCommand(keyCode) as KeyCommand)
          onSetCommandCounter((p) => p + 1)
          event.preventDefault()
          event.stopPropagation()
          return
